...
namespace AnimationEditorTabs
{
	...
	extern const FName MorphTargetListTab;
}

class MORPHVIEWERPLUGIN_API MyAnimationEditor : public IAnimationEditor, public FGCObject, public FEditorUndoClient, public FTickableEditorObject
{
public:
	MyAnimationEditor();
	virtual ~MyAnimationEditor();

	/** Customed */
public:
	static int32 Count;
	TWeakPtr<SDockTab> GetSharedAnimDocumentTab() const { return SharedAnimDocumentTab; }
	TWeakPtr<class MyAnimSequenceCurveEditor> GetCurveEditor() const { return CurveEditor; }
	FName CurveName;
	bool ReOpenAnimationDocumentTab();
	TSharedPtr<class SSequenceEditor> GetSequenceEditor();
	UAnimationAsset* GetAnimationAsset() { return AnimationAsset; }
	
private:
	TSharedPtr<class SSequenceEditor> SequenceEditor;
...
private:
	...
	TWeakPtr<class MyAnimSequenceCurveEditor> CurveEditor;
};
